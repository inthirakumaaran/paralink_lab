...
int main(int argc, char *argv[]) {
    ...
    for (int j =0;j<noOfTimes;j++) {
        ...
        double startSerial=0.0, endSerial=0.0, executionSerial =0.0;
        for (; i < n; i++) {
            int r = rand() % 65536;
            if (!Insert(r, &head)) {
                i--;
...
void PerformSerial() {
    while (cSerialMember<=totalMember || cSerialDelete<=totalDelete || cSerialInsert<=totalInsert  ){
        srand(time(NULL));
        float separator = (rand() % 3);
        int r = rand() % 65536;
        if (separator ==0 && cSerialMember<=totalMember) {
            Member(r, head);
            cSerialMember++;
        } else if (separator == 1 && cSerialInsert<=totalInsert ) {
            Insert(r, &head);
            cSerialInsert++;

        } else if (separator ==2 && cSerialDelete<=totalDelete ){
            Delete(r, &head);
            cSerialDelete++;
        }
    }
...
void Get_args(int argc, char* argv[]) {
    ...
    noOfTimes = (int) strtol(argv[7], NULL, 0);;
